...
export function ContactPage() {
  ...
  return (
    <section className="page-shell section-gap">
      <SectionHeading eyebrow="Contact" title="Help desk and trade support" description="Reach DeArte for catalogue assignments, account activation, and event planning." />
      <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <Panel>
          <div className="grid gap-4">
            <input className="rounded-full border border-white/10 bg-white/5 px-4 py-3" placeholder="Name" />
            <input className="rounded-full border border-white/10 bg-white/5 px-4 py-3" placeholder="Email" />
            <input className="rounded-full border border-white/10 bg-white/5 px-4 py-3" placeholder="Mobile" />
            <select className="rounded-full border border-white/10 bg-white/5 px-4 py-3">
              <option>Buyer Enquiry</option>
              <option>Catalogue Support</option>
              <option>Logistics</option>
            </select>
            <textarea className="min-h-[160px] rounded-[28px] border border-white/10 bg-white/5 p-4" placeholder="Message" />
        ...
        <Panel>
          ...
          <iframe title="DeArte map" src={data.mapsEmbed} className="mt-6 h-[320px] w-full rounded-[28px] border-0" />
        </Panel>
      </div>
    </section>
  );
    ...
    <section className="page-shell section-gap">
      <div className="mb-8 flex flex-wrap gap-3">
        {EDUCATION_ROUTES.map((item) => (
          <Link key={item.slug} to={`/education/${item.slug}`} className={`rounded-full px-4 py-3 text-sm ${item.slug === slug ? 'bg-[var(--color-rose-gold)] text-black' : 'border border-white/10 text-[var(--color-muted)]'}`}>
            {item.title}
          </Link>
        ))}
      </div>
      ...
      <div className="space-y-4">
        {data.map((faq) => (
          <Panel key={faq.id}>
            ...
            <p className="mt-3 text-sm text-[var(--color-muted)]">{faq.answer}</p>
          </Panel>
        ))}
      </div>
    </section>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {data.map((event) => (
          <Panel key={event.id}>
            ...
            <p className="mt-2 text-sm text-[var(--color-champagne)]">{event.date}</p>
        ...
        {data.map((item) => (
          <Panel key={item.id}>
            <div className="flex items-center gap-4">
              <img src={item.avatar} alt={item.name} className="h-16 w-16 rounded-2xl object-cover" />
              <div>
                <h3 className="text-xl font-semibold">{item.name}</h3>
                <p className="text-sm text-[var(--color-muted)]">{item.company}</p>
              </div>
            </div>
            <p className="mt-4 text-sm text-[var(--color-muted)]">{item.review}</p>
          </Panel>
        ))}
      </div>
    </section>
...
export function CareersPage() {
  ...
  return (
    <section className="page-shell section-gap">
      <SectionHeading eyebrow="Careers" title="Join the DeArte team" />
      <div className="space-y-4">
        {data.map((job) => (
          <Panel key={job.id} className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <h3 className="text-2xl font-semibold">{job.title}</h3>
              <p className="mt-2 text-sm text-[var(--color-muted)]">{job.location} • {job.type}</p>
              <p className="mt-3 text-sm text-[var(--color-muted)]">{job.description}</p>
            </div>
...
export function NotFoundPage() {
  return (
    <section className="page-shell section-gap">
      <EmptyState
        title="This route isn’t part of the DeArte collection."
